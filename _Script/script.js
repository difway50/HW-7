function yourGuess() {
    ...
    // Append onto the textarea the result.
      if (guess == numToGuess) {
        guesses.value = guesses.value + "\r" + "Correcto!!! ("+guess+")";
    } else if (guess > numToGuess) {
        guesses.value = guesses.value + "\r" + "Nope... tu ta atra("+guess+")";
    } else {
        guesses.value = guesses.value + "\r" + "Ahhh... nope ("+guess+")";
    }
}
...
function generateNumberToGuess(confirmIt) {
    ...
    guesses.value = "Aqui se generan nuevo to lo digito.\n";
...
function showGuesses(){
    ...
    if (guesses.style.display != 'block') {
        ...
        btn.value = 'Esconde esa vaina';
    } else {
        guesses.style.display = 'none';
        btn.value = 'Enseñamelo otra vez';
    }
}
